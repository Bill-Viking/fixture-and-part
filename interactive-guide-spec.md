...
**Behaviour.** `ClaimNotes` is mounted once in `App.jsx` and drives one popover for all twenty-one: one delegated click listener, one walk at mount that gives each mark its accessible name from its own note. The note is portalled to `<body>` and placed by `usePopoverPlacement` — the same law as the "?" badge, decided against the viewport at the moment it opens: centred under the mark, clamped to the viewport's edges, flipped above when there is no room below. It differs from the badge in one thing, and that is the hook's `follow: false`: the note is `position:absolute` in document coordinates, so it scrolls away with the sentence it belongs to and no scroll listener ever moves it (the badge's popover is `position:fixed` and follows the viewport). The reading reason is that a note answers one sentence and should leave when that sentence does; the measured reason is that a fixed box re-placed on every scroll is a box that moves — one long jump with a note open scored 0.528663 at 390 px and 0.105526 at 1280 before the change, and 0.000000 after. One note at a time; opening a second closes the first. The note is a `role="dialog"` that takes focus when it opens, so Tab walks its sources; Escape closes it and hands focus back to the mark; a click outside closes it. A mark whose id has no note stays inert and warns in a dev build. `__claimCheck()` in a dev build returns `{marks, notes, missing, orphan, unnamed}`.
