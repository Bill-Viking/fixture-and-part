...
- Section 03 closes on a pair of callouts, cool then warm, the machine's side then the human's: `ANALOGY — THE GHOST` (the model as a thing that exists for the length of one pass), and after it `THE AUTHOR — THE SEAM` (the seam, 2026-09-22) — the page's human author in his own words on the seam between one moment and the next and whether anything crosses it, with a short coda in the essay's voice. It carries three claim marks (`parade` THEORY, `sleep` THEORY, `scrapbook` ANALOGY), and the moth callout in Section 08 points at it. It is the only callout on the page written as two paragraphs, which is why the callout body is a block element and `.callout p` carries a 14 px gap rather than the prose's 18.
...
**Behaviour.** `ClaimNotes` is mounted once in `App.jsx` and drives one popover for all twenty-one: one delegated click listener, one walk at mount that gives each mark its accessible name from its own note. The note is portalled to `<body>` and placed by `usePopoverPlacement`, which is the one law every popover on the page obeys — the badge's explanation and the note alike. It is decided against the viewport at the moment it opens: centred under the anchor, clamped to the viewport's edges, below when the box fits below, above when only above fits, and — when it fits neither, being taller than the room on both sides — below at the normal gap, running past the bottom of the viewport with the rest a scroll away, so that it never covers the sentence it is explaining. The answer is then written down in document coordinates: every popover is `position:absolute` and anchored in the page, so it scrolls away with the thing it belongs to, and there is no scroll listener at all. The reading reason is that a note answers one sentence and should leave when that sentence does; the measured reason is that a box re-placed on every scroll is a box that moves — with a note open, one long jump scored 0.528663 at 390 px and 0.105526 at 1280 before the note was anchored, and with a badge open, 300 px of wheel scroll scored 0.017986 at 1280 and 0.092163 at 390 before the badge was (the viewport-following mode was removed, not switched off, in the popover-law pass). Only a resize, and the reflow it starts, moves an open box afterwards, and then it is carried rather than re-decided: the same distance under its own anchor, wherever the anchor went. One note at a time; opening a second closes the first. The note is a `role="dialog"` that takes focus when it opens, so Tab walks its sources; Escape closes it and hands focus back to the mark; a click outside closes it. A mark whose id has no note stays inert and warns in a dev build. `__claimCheck()` in a dev build returns `{marks, notes, missing, orphan}`.
